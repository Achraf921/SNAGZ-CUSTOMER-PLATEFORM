...
import { Card, Tabs, Tab, Button, Form, Container, Row, Col, Alert } from 'react-bootstrap';
import { FaCog, FaStore, FaPalette, FaBoxOpen, FaCubes, FaCheckCircle } from 'react-icons/fa';
...
const ShopifyConfig = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Generating Shopify shop with:', { shopUrl, apiKey, themeName });
        ...
        <Card.Header>
          ...
          >
            <Tab eventKey="settings" title={
              <span><FaCog className="me-2" />Paramètres</span>
            } />
            <Tab eventKey="theme" title={
              <span><FaPalette className="me-2" />Thème</span>
            } />
            <Tab eventKey="products" title={
              <span><FaBoxOpen className="me-2" />Produits</span>
            } />
            <Tab eventKey="extensions" title={
              <span><FaCubes className="me-2" />Extensions</span>
            } />
          </Tabs>
        </Card.Header>
          ...
          {activeTab === 'settings' && (
            <Form onSubmit={handleSubmit}>
              ...
              <div className="d-flex justify-content-end mt-4">
                ...
                >
                  {isGenerating ? 'Génération en cours...' : 'Générer la boutique'}
                </Button>
              </div>
            </Form>
          )}
